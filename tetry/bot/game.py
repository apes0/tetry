...
#        print(frame, data['data']['sent_frame'], data['targetFrame'])
        frame = {'frame': frame, 'type': 'ige', 'data': data}
        print(frame)
    ...
    async def _start(self):  # send a full frame
        skin = 'tetrio'
        opts = {
            **self.opts, **self.context['opts'], 'physical': True,
            'username': self.bot.name, 'ghostskin': skin, 'minoskin':
                {mino: skin for mino in [
                    'i', 'j', 'l', 'o', 's', 't', 'z', 'other', ]},
            'boardskin': 'generic', 'boardwidth': 10, 'boardheight': 20,
            'boardbuffer': 20  # there *MUST* be a way to get these
        }
        self.pendingFrames.append(
            {
                'data': {  # fire and gameoverreason seem to be optional
                    ...
                    'replay': {},
                    'source': {},
                    'stats': {'seed': self.seed, 'lines': 0, 'level_lines': 0, 'level_lines_needed': 1, 'inputs': 0, 'holds': 0, 'time': {'start': 0, 'zero': True, 'locked': False, 'prev': 0, 'frameoffset': 0}, 'score': 0, 'zenlevel': 1, 'zenprogress': 0, 'level': 1, 'combo': 0, 'currentcombopower': 0, 'topcombo': 0, 'btb': 0, 'topbtb': 0, 'tspins': 0, 'piecesplaced': 0, 'clears': {'singles': 0, 'doubles': 0, 'triples': 0, 'quads': 0, 'realtspins': 0, 'minitspins': 0, 'minitspinsingles': 0, 'tspinsingles': 0, 'minitspindoubles': 0, 'tspindoubles': 0, 'tspintriples': 0, 'tspinquads': 0, 'allclear': 0}, 'garbage': {'sent': 0, 'received': 0, 'attack': 0, 'cleared': 0}, 'kills': 0, 'finesse': {'combo': 0, 'faults': 0, 'perfectpieces': 0}},
                    'targets': [],
                    'fire': 0,
